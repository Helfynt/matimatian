#!/bin/bash
# coded by @Tn.pennyw1s3 
 
banner() {
 
printf "\e[1;97m"
printf " _______  _       __________________ _______  _______  _       _________ _______ _________\n"
printf "(  ___  )( (    /|\__   __/\__   __/(  ____ \(  ___  )( (    /|\__   __/(  ____ \\__   __/\n"
printf "| (   ) ||  \  ( |   ) (      ) (   | (    \/| (   ) ||  \  ( |   ) (   | (    \/   ) (   \n"
printf "| (___) ||   \ | |   | |      | |   | (_____ | (___) ||   \ | |   | |   | (__       | |   \n"
printf "|  ___  || (\ \) |   | |      | |   (_____  )|  ___  || (\ \) |   | |   |  __)      | |   \n"
printf "| (   ) || | \   |   | |      | |         ) || (   ) || | \   |   | |   | (         | |   \n"
...
checkroot() {
    if [[ "$(id -u)" -ne 0 ]]; then
      printf ".:: Program ini berjalan di perangkat yg udah di root boejang!\n"
      exit 1
    fi
}
readconfig() {
...
if [[ "$checkicmp" == "0" ]]; then
printf "\e[1;77m:: Jatuhkan icmp (permintaan ping)\n\e[0m"
fi
 
if [[ "$checkudp" == "0" ]]; then
printf "\e[1;77m:: Anti Banjir UDP\e[0m\n"
fi
 
if [[ "$checkfragchains" == "0" ]]; then
printf "\e[1;77m:: Jatuhkan fragmen di semua rantai\e[0m\n"
fi
 
if [[ "$checksourceip" == "0" ]]; then
printf "\e[1;77m:: Batasi koneksi per ip sumber\e[0m\n"
fi
 
if [[ "$checkrst" == "0" ]]; then
printf "\e[1;77m:: Batasi paket RST\n\e[0m"
fi
 
if [[ "$checkinvalid" == "0" ]]; then
printf "\e[1;77m:: Jatuhkan paket yang tidak valid\n\e[0m"
fi
 
if [[ "$checknew" == "0" ]]; then
printf "\e[1;77m:: Jatuhkan paket tcp yang baru dan bukan SYN\n\e[0m"
fi
 
if [[ "$checkmss" == "0" ]]; then
printf "\e[1;77m:: Jatuhkan paket SYN dengan nilai suspek MSS\n\e[0m"
fi
 
if [[ "$checksourcesec" == "0" ]]; then
printf "\e[1;77m:: Batasi koneksi TCP baru per detik per IP sumber\n\e[0m"
fi
 
if [[ "$checkbogus" == "0" ]]; then
printf "\e[1;77m:: Memblokir paket dengan flag TCP palsu\n\e[0m"
fi
 
if [[ "$checspoof" == "0" ]]; then
printf "\e[1;77m:: Blokir paket palsu\n\e[0m"
fi
...
# icmp
read -p $'\e[1;77mBlokir permintaan icmp (ping) [Y/n]: \e[0m' ricmp
...
# udp
default_rudpflood="y"
read -p $'\e[1;77mAnti Banjir UDP [Y/n]: \e[0m' rudpflood
...
# chains
default_rchains="y"
read -p $'\e[1;77mJatuhkan fragmen di semua rantai [Y/n]: \e[0m' rchains
#="${:-${default_}}"
...
#read -p " [Y/n]: " 
default_rsource="y"
read -p $'\e[1;77mBatasi koneksi per ip sumber [Y/n]: \e[0m' rsource
...
# rst
default_rrst="y"
read -p $'\e[1;77mBatasi paket RST [Y/n]: \e[0m' rrst
rrst="${rrst:-${default_rrst}}"
printf "rst=$rrst\n" >> /etc/antiflood.cfg
 
 
# jatuhkan paket yang tidak valid
default_rinvalid="y"
read -p $'\e[1;77mjatuhkan paket yang tidak valid [Y/n]: \e[0m' rinvalid
rinvalid="${rinvalid:-${default_rinvalid}}"
printf "invalid=$rinvalid\n" >> /etc/antiflood.cfg
 
 
# jatuhkan paket tcp yang baru dan bukan SYN
# ="${:-${default_}}"
default_rnew="y"
read -p $'\e[1;77mjatuhkan paket tcp yang baru dan bukan SYN [Y/n]: \e[0m' rnew
rnew="${rnew:-${default_rnew}}"
printf "new=$rnew\n" >> /etc/antiflood.cfg
 
 
# jatuhkan paket SYN dengan nilai suspek MSS
default_rmss="y"
read -p $'\e[1;77mjatuhkan paket SYN dengan nilai suspek MSS [Y/n]: \e[0m' rmss
rmss="${rmss:-${default_rmss}}"
printf "mss=$rmss\n" >> /etc/antiflood.cfg
 
 
# batasi koneksi TCP baru per detik per IP sumber
default_rsourcesec="y"
read -p $'\e[1;77mbatasi koneksi TCP baru per detik per IP sumber [Y/n]: \e[0m' rsourcesec
rsourcesec="${rsourcesec:-${default_rsourcesec}}"
printf "sourceipsec=$rsourcesec\n" >> /etc/antiflood.cfg
 
 
# Memblokir paket dengan flag TCP palsu
default_rbogus="y"
read -p $'\e[1;77mMemblokir paket dengan flag TCP palsu [Y/n]: \e[0m' rbogus
...
# Blokir paket palsu
default_rspoof="y"
read -p $'\e[1;77mBlokir paket palsu [Y/n]: \e[0m' rspoof
